...
def HST(ori_samples, l, h, eps, target_value, fake_user_num, post_type, domain_bins):
    ...
    noisy_result = np.random.choice([-1., 1.], [n, domain_bins], replace=True)  # random matrix in HST
    # print(noisy_result[-fake_user_num:, :])


    if np.size(target_value) != 0:  # the last column is 1, the other columns are -1
        bad_vector = [-1] * domain_bins
        bad_vector[-1] = 1
        bad_vector = np.array(bad_vector)

        noisy_result[-fake_user_num:, :] = bad_vector
...
if __name__ == "__main__":
    ...
    fake_user_num = int(n * 0.01)

    result = HST(data, l, h, eps, target_value, fake_user_num, post_type, bins)
